...
`r shiny::icon('map-signs')` Get started with building a model in this R Markdown document that accompanies [Evaluate your model with resampling](https://www.tidymodels.org/start/resampling) tidymodels start article.
...
This time we don't need to preprocess the data as much, therefore we will not use a **recipe**.
Let's create the model specification for a random forest model, using `ranger` engine and set mode to `classification`.
